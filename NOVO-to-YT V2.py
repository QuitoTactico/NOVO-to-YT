...
def ver_puntos():  p.moveTo(670, 325, 0.5)

def terminal():     p.moveTo(690, 625, 0.2)
...
def imagen_click(image, confidence = 0.8, duration = 0.2, region = None, minSearchTime = None):
    try:
        p.click(
            p.locateCenterOnScreen(f'images/{image}.PNG', 
...
lista_de_prueba = {'-Prueba-': ['La costa del silencio',
...
def escoger(novos):
    while True:
        ...
        carpeta = input()
        if carpeta == '0': break
        if carpeta.isnumeric():     carpeta = f'-Novo {carpeta}-'
        if carpeta[0] != '-':       carpeta = f'-{carpeta}-'
        print(carpeta)
        
        when = 'Está en la lista, hora del sexo' if carpeta in novos.keys() else 'Eres un imbécil'
        print(when, '\n')
