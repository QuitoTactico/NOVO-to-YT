...
dir = "INPUT/ListaTusArchivos-Script-v4 - Musica.csv"
...
def sussy_baka(lista: list) -> None:
    ...
    for num in range(len(lista)):
        print(
            f"{num}/{len(lista)} | {round(((num+1)/len(lista)) *100 , 2)}% | {lista[num]}"
        )
        ...
        # exceptions
        if exception_confirmar("para_ti"):
            continue
...
def cerrar():
    p.click(1344, 15, duration=0.2)


def cola():
    p.click(606, 263, duration=0.2)


def terminal():
    p.moveTo(
        p.locateCenterOnScreen("images/Terminal.PNG", confidence=0.9), duration=0.1
    )


def opera():
    p.click(p.locateCenterOnScreen("images/Opera.PNG", confidence=0.9), duration=0.2)


def visual():
    p.click(p.locateCenterOnScreen("images/Visual.PNG", confidence=0.9), duration=0.2)


def buscar_texto():
    p.click(p.locateCenterOnScreen("images/Buscar.PNG", confidence=0.9), duration=0.2)


def buscar_x():
    p.click(p.locateCenterOnScreen("images/Buscar_x.PNG", confidence=0.9), duration=0.2)
...
def lupa():
    p.click(p.locateCenterOnScreen("images/Lupa.PNG", confidence=0.9), duration=0.2)
...
def menu_cola(level: int = 2, x: int = 1306, ex: bool = False):
    try:
        sleep(0.7)
        p.click(
            p.locateCenterOnScreen("images/Menu_cola.PNG", confidence=0.8), duration=0.3
        )
    except:
        if level == 1:
            points_up(x)
        ...
        sleep(0.5)
        p.click(
            p.locateCenterOnScreen("images/Menu_cola_icon.PNG", confidence=0.9),
            duration=0.4,
        )


def points_up(x=1306):
    p.click(x + 4, 204, duration=0.3)


def points(x=1306, ex=False):
    if ex:
        p.click(x, 295, duration=0.3)
    else:
        p.click(x, 237, duration=0.3)


def points_down(x=1306):
    p.click(x, 304, duration=0.3)
...
if __name__ == "__main__":
    ...
    while True:
        ...
        if lista_actual == "0":
            break
        if lista_actual.isnumeric():
            lista_actual = f"-Novo {lista_actual}-"
        if lista_actual[0] != "-":
            lista_actual = f"-{lista_actual}-"
        print(lista_actual)

        when = (
            "Está en la lista, hora del sexo"
            if lista_actual in novos.keys()
            else "Eres un imbécil"
        )
        print(when, "\n")

        if when != "Eres un imbécil":
            sussy_baka(novos[lista_actual])
